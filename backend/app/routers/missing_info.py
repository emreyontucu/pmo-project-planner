...
from ..models import Project, ProjectAnswer
from .. import schemas

# Rule-based (no LLM) "eksik bilgi" questionnaire: never re-asks data already present,
# only surfaces required fields left blank (task descriptions) plus project-type-relevant
# questions the Görev Takip Excel has no column for. Kept separate from routers/questions.py
# (now the WBS Wizard, a different feature: suggests whole new task packages rather than
# filling gaps in existing tasks) so the two don't collide on route paths or intent.
router = APIRouter(prefix="/projects/{project_id}/questions", tags=["missing-info"])
...
def _description_gap_key(task_id: int) -> str:
    return f"task:{task_id}:description"
...
async def get_pending_questions(project_id: int, db: AsyncSession = Depends(get_db)):
    ...
    for task in project.tasks:
        if task.status == "İptal Edildi":
            continue
        if not task.description:
            pending.append(schemas.PendingQuestion(
                type="field_gap",
                key=_description_gap_key(task.id),
